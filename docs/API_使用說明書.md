...
> **Auth 注意**：除 `GET /health`、`GET /system/ui-locale`、`POST /auth/register`、`POST /auth/login`、`POST /auth/bypass` 外，API 需登入後使用 HttpOnly Cookie。非 GET/HEAD/OPTIONS 請求還需帶 `X-CSRF-Token` header，值來自登入/註冊回應的 `csrf_token`。
...
```bash
# 步驟 1：登入並保存 Cookie；從回應 JSON 取出 csrf_token
curl -c .pyTestTemp/cookies.txt -X POST http://localhost:8088/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "owner", "password": "YOUR_PASSWORD"}'

# 步驟 2：建立 Session（替換 CSRF_TOKEN）
curl -b .pyTestTemp/cookies.txt -X POST http://localhost:8088/api/v1/session \
  -H "X-CSRF-Token: CSRF_TOKEN"

# 步驟 3：發送訊息（替換 CSRF_TOKEN / SESSION_ID）
curl -b .pyTestTemp/cookies.txt -X POST http://localhost:8088/api/v1/chat/sync \
  -H "Content-Type: application/json" \
  -H "X-CSRF-Token: CSRF_TOKEN" \
...
```python
import requests
r = requests.get("http://localhost:8088/api/v1/health")
print(r.json())
```
...
```bash
curl -X POST http://localhost:8088/api/v1/session
```

### `GET /session/{session_id}` — 取得 Session 狀態

```bash
curl http://localhost:8088/api/v1/session/a1b2c3d4-e5f6-...
```

### `DELETE /session/{session_id}` — 刪除 Session

```bash
curl -X DELETE http://localhost:8088/api/v1/session/a1b2c3d4-e5f6-...
```
...
```bash
curl -X POST http://localhost:8088/api/v1/session/a1b2c3d4-e5f6-.../bridge
```
...
| `display_content` | string | ❌ | null | 顯示用訊息；未提供時使用 `content` |
| `session_id` | string | ❌ | null | 不帶則自動建立新 Session |
| `character_ids` | string[] | ❌ | null | 指定單聊或群聊角色清單 |
| `group_name` | string | ❌ | null | 群聊名稱 |
| `channel` / `channel_uid` | string | ❌ | null | 外部平台 channel scope |
| `channel_class` | `"public"` / `"private"` | ❌ | null | 部署通道類型 |
| `persona_face` | `"public"` / `"private"` | ❌ | null | 人格/記憶可見面 |
| `external_context` | object | ❌ | null | transient 外部上下文，不寫入 private memory；可用 `persist_visible_event=false` 跳過可見 system event |
| `transient_context` | object | ❌ | null | app runtime context，只注入 final chat prompt；`context_text` 不寫入 session history / memory extraction，且不可與 `external_context` 同時使用 |
| `include_speech` | bool | ❌ | true | 是否回傳 TTS/speech 欄位 |
| `memory_write_policy` | `"normal"` / `"transient"` | ❌ | `"normal"` | 記憶寫入策略 |
| `tool_routing_policy` | `"auto"` / `"disabled"` | ❌ | `"auto"` | 本次 request 是否允許 chat orchestration 自動判斷並執行工具 |

#### `tool_routing_policy` contract

`tool_routing_policy` 控制的是 `/chat/sync` 與 `/chat/stream-sync` 內部的自動工具路由：

- `"auto"`：預設行為。若系統設定中有可用工具，chat orchestration 可先呼叫 Router Agent，再執行 tavily / weather / bash / browser / image 等工具。
- `"disabled"`：本次 request 不提供任何工具給自動路由，單層與雙層 chat orchestration 都會直接進入一般回覆生成；不會跑 Router Agent，也不會透過 chat orchestration 執行工具。

停用 tool routing 不等於全域停用工具模組。它只影響這次 chat request 的自動工具判斷；其他明確的 API（例如 system weather cache）或後端程式碼直接呼叫工具函式時，不會被此欄位攔截。目前 MemoriaCore 沒有公開的「任意 tool direct-call」REST endpoint；外部專案若要直接呼叫特定工具，應新增一個明確授權、明確參數 schema 的專用 endpoint，而不是透過 `/chat/sync` 的自動 routing。

外部專案只想取得角色回覆、不想觸發工具時：

```json
{
  "content": "請根據我提供的上下文回覆，不要自行查資料。",
  "session_id": "a1b2c3d4-...",
...
- `external_context.router_context` 可選；若提供，MemoriaCore 會將它正規化為本輪 `group_router` 的 compact `router_turn_context`，供內部組成本輪任務與 speaker/action routing 使用。
- `router_context.summary` / `router_context.instruction` 只用來組 `turn_state_json.original_user_request`；不會直接渲染到 `<external_turn_context_json>`。
- `<external_turn_context_json>` 是 router 可見的補充背景 projection，只輸出 `routing_hint` 與 `context_excerpt`。
- 若未提供 `external_context.router_context`，MemoriaCore 會從 `external_context.context_text` 嘗試抽取 `Event summary:` / `Event instruction:` 等簡短路由資訊作為內部任務；`context_excerpt` 不會無條件複製完整 `context_text`。
- PersonaCore world event 的 fallback `context_excerpt` 只保留 `[PersonaCore 場景感知]` 之後的場景資料，並排除 `[PersonaCore world event]`、`Event summary:` 與 `Event instruction:`。
- `router_turn_context` 不是 `previous_context`，不代表歷史訊息，也不會被保存成 user message。
- `persist_visible_event=false` 仍只控制是否建立可見 system event；它不會阻止 router 使用 compact `router_turn_context` 判斷本輪外部事件。
- 有 `external_context` 時，本輪不執行 query expansion、topic shift detection 或 memory retrieval，避免系統事件 / 外部觸發文字污染使用者意圖判斷。
...
{
  "content": "角色主動回合。",
  "character_ids": ["default", "5d18512d-359f-4719-9763-38f325fc1521"],
  "external_context": {
    "source": "personacore_world_event",
...
`transient_context` 是給 PersonaCore 這類 app integration 使用的本輪暫態 runtime context。它只進 final chat LLM prompt，不進 router / expand / tool routing。`transient_context.context_text` 不會寫入 MemoriaCore session history，也不進 memory extraction；一般 user message 仍會依 request policy 照常保存與抽取。

同一個 request body contract 也適用於 `POST /chat/stream-sync`；`POST /chat/sync` 與 `POST /chat/stream-sync` 共用 `api/routers/chat/execution.py` 的 prepare flow。

欄位：

- `source?: string`：debug / summary 用來源標籤。
- `context_text: string`：本輪 runtime context 內容。
- `max_chars?: int`：本次 request 的裁切上限，仍受硬上限限制。
...
- `transient_context.context_text` 不寫入 session history / memory extraction；但和其他 final chat prompt 內容一樣，啟用 LLM trace 時，渲染後的 `<runtime_context>` 可能出現在 `runtime/llm_trace.jsonl`。該檔是 privileged diagnostic log，不是對話歷史或長期記憶。
- `external_context` 和 `transient_context` 互斥；同時傳會回 HTTP 400。
...
login = session.post("http://localhost:8088/api/v1/auth/login", json={
    "username": "owner",
    "password": "YOUR_PASSWORD",
}).json()
headers = {"X-CSRF-Token": login["csrf_token"]}

resp = session.post("http://localhost:8088/api/v1/chat/sync", headers=headers, json={
    "content": "我最近在學鋼琴",
    "session_id": "a1b2c3d4-..."
})
data = resp.json()
...
目前 WebSocket 也會驗證登入 Cookie；非瀏覽器客戶端需先完成 `/auth/login`，並在 WebSocket handshake 帶上同一組 Cookie。

**連線範例**:
```
ws://localhost:8088/api/v1/chat/stream?session_id=a1b2c3d4-...
```
...
async def chat():
    uri = "ws://localhost:8088/api/v1/chat/stream"
...
```bash
curl "http://localhost:8088/api/v1/memory/blocks"
curl "http://localhost:8088/api/v1/memory/blocks?include_vectors=true"
```

### `GET /memory/blocks/{block_id}` — 取得單一記憶區塊

```bash
curl http://localhost:8088/api/v1/memory/blocks/blk_abc123
```

### `PUT /memory/blocks/{block_id}` — 更新區塊概覽

```bash
curl -X PUT http://localhost:8088/api/v1/memory/blocks/blk_abc123 \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE http://localhost:8088/api/v1/memory/blocks/blk_abc123
```
...
```bash
curl -X POST http://localhost:8088/api/v1/memory/search \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://localhost:8088/api/v1/memory/expand-query \
  -H "Content-Type: application/json" \
...
```bash
curl http://localhost:8088/api/v1/memory/core
```
...
```bash
curl -X POST http://localhost:8088/api/v1/memory/core/search \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE http://localhost:8088/api/v1/memory/core/core_001
```
...
```bash
curl http://localhost:8088/api/v1/profile
curl "http://localhost:8088/api/v1/profile?include_tombstones=true"
```
...
```bash
curl http://localhost:8088/api/v1/profile/喜歡的音樂類型
```

### `PUT /profile/{fact_key}` — 新增或更新事實

```bash
curl -X PUT http://localhost:8088/api/v1/profile/喜歡的音樂類型 \
  -H "Content-Type: application/json" \
  -d '{
    "fact_value": "爵士樂",
    "category": "preference",
...
```bash
curl -X DELETE http://localhost:8088/api/v1/profile/喜歡的音樂類型
```

### `POST /profile/search` — 語意搜尋偏好

```bash
curl -X POST http://localhost:8088/api/v1/profile/search \
  -H "Content-Type: application/json" \
...
```bash
curl http://localhost:8088/api/v1/profile/static-prompt
```
...
```bash
curl "http://localhost:8088/api/v1/memory/graph?similarity_threshold=0.5"
```
...
```bash
curl http://localhost:8088/api/v1/system/config
```
...
```bash
curl -X PUT http://localhost:8088/api/v1/system/config \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://localhost:8088/api/v1/system/consolidate \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://localhost:8088/api/v1/system/preference-aggregate \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://localhost:8088/api/v1/system/synthetic \
  -H "Content-Type: application/json" \
...
```bash
curl "http://localhost:8088/api/v1/logs?limit=20&offset=0&type=llm_call"
```

### `DELETE /logs` — 清除所有日誌

```bash
curl -X DELETE http://localhost:8088/api/v1/logs
```

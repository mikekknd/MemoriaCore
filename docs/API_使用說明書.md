...
- `external_context.router_context` 可選；若提供，MemoriaCore 會將它正規化為本輪 `group_router` 可見的 compact `router_turn_context`，供 speaker/action routing 使用。
- 若未提供 `external_context.router_context`，MemoriaCore 會從 `external_context.context_text` 嘗試抽取 `Event summary:` / `Event instruction:` 等簡短路由資訊。
- `router_turn_context` 只進入 group router prompt；它不是 `previous_context`，不代表歷史訊息，也不會被保存成 user message。

...
async def list_snapshots(character_id: str = Query(..., description="角色 ID")):
    """回傳該角色所有 snapshot 摘要（不含 dimensions 內容），版本遞增排序。"""
    sto = get_storage()
    rows = sto.list_persona_snapshots(character_id)
    return [PersonaSnapshotSummaryDTO(**r) for r in rows]


@router.get("/snapshots/latest", response_model=PersonaSnapshotDTO)
async def get_latest_snapshot(character_id: str = Query(..., description="角色 ID")):
    """回傳該角色最新版 snapshot（含所有 dimensions）；無紀錄時回 404。"""
    sto = get_storage()
    raw = sto.get_latest_persona_snapshot(character_id)
    if raw is None:
...
async def get_latest_snapshot_tree(character_id: str = Query(..., description="角色 ID")):
    ...
    tree = store.get_latest_tree(character_id)
    if tree is None:
        raise HTTPException(status_code=404, detail="no snapshot for character")
...
async def get_snapshot(version: int, character_id: str = Query(..., description="角色 ID")):
    """回傳指定版本的 snapshot（含 dimensions）；不存在回 404。"""
    sto = get_storage()
    raw = sto.get_persona_snapshot(character_id, version)
    if raw is None:
...
async def get_snapshot_tree(version: int, character_id: str = Query(..., description="角色 ID")):
    """回傳指定版本的 Force-Directed Graph 結構。"""
    store = get_persona_snapshot_store()
    tree = store.get_tree(character_id, version)
    if tree is None:
        raise HTTPException(status_code=404, detail="snapshot not found")
    return _tree_to_dto(tree)
...
async def list_traits(
    ...
    """
    sto = get_storage()
    rows = sto.get_active_traits(character_id) if active_only else sto.get_all_traits(character_id)
...
async def get_trait_timeline(
    ...
    """
    sto = get_storage()
    points = sto.get_trait_timeline(character_id, trait_key)

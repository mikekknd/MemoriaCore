...
async def test_final_closing_prefetch_consume_uses_dedicated_no_chain_policy(monkeypatch):
    with temp_storage() as storage:
        ...
        manager = YouTubeBridgeManager(storage, memoria_client_factory=FakeClosingMemoriaClient)
        manager._runtimes["live-a"] = runtime
        consume_calls: list[turn_pipeline.PreparedTurnConsumeOptions] = []
        original_consume_prepared_turn = turn_pipeline.consume_prepared_turn

        async def spy_consume_prepared_turn(adapter, payload, options):
            consume_calls.append(options)
            return await original_consume_prepared_turn(adapter, payload, options)
        ...
        def fake_policy(interaction_arg):
            ...
            return type("SentinelPolicy", (), {
                ...
                "presentation_source": "sentinel_closing",
                "may_chain": False,
                "mark_audience_events_injected": False,
                "dedicated_closing": True,
            })()

        monkeypatch.setattr(engine_closing, "consume_prepared_turn", spy_consume_prepared_turn, raising=False)
        interaction = storage.create_interaction({
            "session_id": "live-a",
            "source": "director_prefetch",
            "priority": 40,
        ...
        async def fake_present_prepared(_session_id, prepared_results, *, source, interaction_job_id=""):
            ...
            assert source == "sentinel_closing"
        ...
        monkeypatch.setattr(turn_pipeline, "prepared_turn_policy_for_interaction", fake_policy)
        monkeypatch.setattr(manager, "present_prepared_stream_results", fake_present_prepared)
        monkeypatch.setattr(
            manager,
        ...
        assert consume_calls[0].expected_dedicated_closing is True
        assert consume_calls[0].allow_followup_prefetch is False
        assert consume_calls[0].completion_metadata_key == "final_closing_prefetch_consumed"
        assert seen_sources == ["sentinel_closing"]
        assert storage.get_interaction(interaction["job_id"])["status"] == "completed"


@pytest.mark.asyncio
async def test_final_closing_prefetch_presentation_exception_cancels_and_returns_none(monkeypatch):
    with temp_storage() as storage:
        storage.upsert_connector({"connector_id": "yt", "name": "YouTube", "api_key": "key", "enabled": True})
        session = storage.upsert_session({
            "session_id": "live-a",
        ...
        async def fail_present(*_args, **_kwargs):
            raise RuntimeError("presentation boom")

        monkeypatch.setattr(manager, "present_prepared_stream_results", fail_present)
        ...
        cancelled = storage.get_interaction(interaction["job_id"])
        assert cancelled["status"] == "interrupted"
        assert cancelled["reason"] == "final_closing_prefetch_consume_failed"
        cancelled_item = storage.get_presentation_item(item["item_id"])
        assert cancelled_item["status"] == "cancelled"
        assert cancelled_item["error"] == "final_closing_prefetch_consume_failed"
...
async def test_finalize_refreshes_final_closing_prefetch_when_drain_later_exposes_new_target(monkeypatch):
    with temp_storage() as storage:
        ...
        assert len(calls) == 1
        assert "drain 稍後的新最後一句。" in calls[0]
        final_prefetches = [
            interaction
            for interaction in storage.list_interactions("live-a")
            if (
...
@pytest.mark.asyncio
async def test_finalize_defers_final_closing_prefetch_until_active_director_reveals_visible_target(monkeypatch):
    with temp_storage() as storage:
        storage.upsert_connector({"connector_id": "yt", "name": "YouTube", "api_key": "key", "enabled": True})
        session = storage.upsert_session({
            "session_id": "live-a",
        ...
        async def reveal_active_visible_target():
            await asyncio.sleep(0.02)
            item = storage.create_presentation_item({
                "session_id": "live-a",
                "interaction_job_id": active["job_id"],
                "message_id": "active-director:0",
                "character_id": "char-a",
                "character_name": "角色A",
                "sequence_index": 0,
                "text": "active director 播放中的新最後一句。",
                "status": "presenting",
                "audio_path": "active-director.wav",
                "audio_format": "wav",
                "presented_at": datetime.now().isoformat(),
            ...
            storage.append_interaction_visible_message(
                active["job_id"],
                {
                    "message_id": "active-director:0",
                    "role": "assistant",
                    "content": "active director 播放中的新最後一句。",
                    "timestamp": datetime.now().isoformat(),
                    "character_id": "char-a",
                    "character_name": "角色A",
                    "source": "director",
        ...
        async def fake_send_director_turn(session_arg, state_arg, decision_arg, *, prefetch_only=False, **_kwargs):
            assert decision_arg["action"] == "final_closing"
            if not prefetch_only:
                raise AssertionError("final closing should consume the visible-target prefetch instead of regenerating")
            prompt = decision_arg["prompt"]
            calls.append(prompt)
            assert "active director 播放中的新最後一句。" in prompt
            assert "原本最後一句。" not in prompt
            interaction = storage.create_interaction({
                "session_id": "live-a",
                "source": "director_prefetch",
                "priority": 40,
                ...
                "character_ids": ["char-a"],
                "content": prompt,
                "reply_text": "承接 active director 的正式收尾。",
                "metadata": {
                    "prefetch_only": True,
                    "decision": decision_arg,
                    "base_state": state_arg,
            ...
            item = storage.create_presentation_item({
                "session_id": "live-a",
                "interaction_job_id": interaction["job_id"],
                "message_id": "final-prefetch:active",
                "character_id": "char-a",
                "character_name": "角色A",
                "sequence_index": 0,
                "text": "承接 active director 的正式收尾。",
                "status": "ready",
                "audio_path": "final-active.wav",
                "audio_format": "wav",
                "metadata": {"source": "director_prefetch"},
            })
            return {
                ...
                "memoria_result": {
                    "session_id": "mem-a",
                    "message_id": 91,
                    "reply": "承接 active director 的正式收尾。",
                },
                "prepared_results": [{
                    "message": {
                        "message_id": "final-prefetch:active",
                        "content": "承接 active director 的正式收尾。",
                    },
                    "items": [item],
                }],
                "decision": decision_arg,
        ...
        reveal_task = asyncio.create_task(reveal_active_visible_target())
        try:
            await manager._finalize_live_session(
                runtime,
                session,
        ...
        assert len(calls) == 1
        assert "active director 播放中的新最後一句。" in calls[0]
        final_prefetches = [
            interaction
            for interaction in storage.list_interactions("live-a")
            if (
        ...
        assert len(final_prefetches) == 1
        assert final_prefetches[0]["status"] == "completed"
        assert final_prefetches[0]["metadata"]["final_closing_prefetch_consumed"] is True
...
@pytest.mark.asyncio
async def test_finalize_prefetches_closing_super_chat_thanks_for_current_drain_target_despite_active_audience_prepare(monkeypatch):
    with temp_storage() as storage:
        storage.upsert_connector({"connector_id": "yt", "name": "YouTube", "api_key": "key", "enabled": True})
        session = storage.upsert_session({
            "session_id": "live-a",
        ...
        current_interaction = storage.create_interaction({
            "session_id": "live-a",
            "source": "director",
            "priority": 50,
            "status": "completed",
            "memoria_session_id": "mem-a",
            "character_ids": ["char-a"],
            "content": "current planned turn",
            "metadata": {},
        })
        current_item = storage.create_presentation_item({
            "session_id": "live-a",
            "interaction_job_id": current_interaction["job_id"],
            "message_id": "current:0",
            "character_id": "char-a",
            "character_name": "角色A",
            "sequence_index": 0,
            "text": "正在播放、closing thanks 應該承接的句子。",
            "status": "presenting",
        ...
        storage.create_interaction({
            ...
            "memoria_session_id": "mem-a",
            "character_ids": ["char-a"],
            "content": "stuck audience prepare",
            "metadata": {
                "prepare_only": True,
                "decision": {"action": "reply_chat_batch"},
                "base_state": {},
        ...
        async def fake_send_director_turn(session_arg, state_arg, decision_arg, *, prefetch_only=False, **_kwargs):
            ...
            if decision_arg["action"] == "closing_super_chat_thanks":
                if not prefetch_only:
                    raise AssertionError("closing SC thanks should use the prefetch started before drain")
                assert "正在播放、closing thanks 應該承接的句子。" in decision_arg["prompt"]
                reply_text = "承接目前播放句子的 SC 感謝。"
                interaction = storage.create_interaction({
                    "session_id": "live-a",
                    "source": "director_prefetch",
                    "priority": 40,
                ...
                return {
                    "interaction": interaction,
                    "memoria_result": {"session_id": "mem-a", "message_id": 90, "reply": reply_text},
                    "prepared_results": [{
                        "message": {"message_id": "sc-prefetch", "content": reply_text},
                        "items": [item],
                    }],
                    "decision": decision_arg,
                    "base_state": state_arg,
            ...
            assert decision_arg["action"] == "final_closing"
            if not prefetch_only:
                raise AssertionError("final closing should consume prefetched result after SC thanks")
            reply_text = "正式收尾。"
            interaction = storage.create_interaction({
                "session_id": "live-a",
                "source": "director_prefetch",
            ...
            return {
                "interaction": interaction,
                "memoria_result": {"session_id": "mem-a", "message_id": 91, "reply": reply_text},
                "prepared_results": [{
                    "message": {"message_id": "final-prefetch", "content": reply_text},
                    "items": [item],
                }],
                "decision": decision_arg,
                "base_state": state_arg,
        ...
        async def fake_drain(_runtime_arg, _session_arg, **_kwargs):
            assert calls and calls[0][0:2] == ("closing_super_chat_thanks", True)
            storage.update_presentation_item(
                current_item["item_id"],
                status="played",
                presented_at=current_item["presented_at"],
                acked_at=datetime.now().isoformat(),
            )
            storage.append_interaction_visible_message(
                current_interaction["job_id"],
                {
                    "message_id": current_item["message_id"],
                    "role": "assistant",
                    "content": current_item["text"],
                    "timestamp": datetime.now().isoformat(),
                    "character_id": current_item["character_id"],
                    "character_name": current_item["character_name"],
                    "source": "director",
                },
            )
...
@pytest.mark.asyncio
async def test_finalize_prefetches_sc_thanks_during_drain_when_only_normal_safety_pending(monkeypatch):
    with temp_storage() as storage:
        storage.upsert_connector({"connector_id": "yt", "name": "YouTube", "api_key": "key", "enabled": True})
        session = storage.upsert_session({
            "session_id": "live-a",
        ...
        sc_event = storage.save_event({
            "bridge_session_id": "live-a",
            "connector_id": "yt",
            "youtube_message_id": "sc-ready",
            "message_type": "superChatEvent",
            "author_display_name": "SC觀眾",
            "message_text": "SC 已完成 safety。",
            "amount_display_string": "NT$150",
            "priority_class": "super_chat",
            "safety_status": "completed",
            "safety_label": "clean",
            "safe_message_text": "SC 已完成 safety。",
        })
        storage.save_event({
            "bridge_session_id": "live-a",
            "connector_id": "yt",
            "youtube_message_id": "normal-pending",
            "message_type": "testMessageEvent",
            "author_display_name": "普通觀眾",
            "message_text": "普通留言還在等 safety。",
            "priority_class": "normal",
            "safety_status": "pending",
            "safety_label": "",
            "safe_message_text": "",
        })
        drain_interaction = storage.create_interaction({
            "session_id": "live-a",
            "source": "director_audience_prepare",
            "priority": 45,
            "status": "prepared",
        ...
        drain_item = storage.create_presentation_item({
            "session_id": "live-a",
            "interaction_job_id": drain_interaction["job_id"],
            "message_id": "audience-drain:0",
            "character_id": "char-a",
            "character_name": "角色A",
            "sequence_index": 0,
            "text": "drain 期間仍在播放的普通回覆最後一句。",
            "status": "ready",
            "audio_path": "audience.wav",
            "audio_format": "wav",
            "metadata": {"source": "director_audience_prepare"},
        ...
        safety_can_finish = asyncio.Event()

        async def fake_resolve_pending_safety(session_id, **_kwargs):
            assert session_id == "live-a"
            safety_started.set()
            await safety_can_finish.wait()
            return {"status": "completed", "initial_pending_count": 1, "classified_count": 1}
        ...
        async def fake_send_director_turn(session_arg, state_arg, decision_arg, *, prefetch_only=False, **_kwargs):
            ...
            assert decision_arg["action"] == "closing_super_chat_thanks"
            if not prefetch_only:
                raise AssertionError("completed SC safety should allow SC thanks prefetch instead of sync generation")
            assert "drain 期間仍在播放的普通回覆最後一句。" in decision_arg["prompt"]
            interaction = storage.create_interaction({
                "session_id": "live-a",
                "source": "director_prefetch",
                "priority": 40,
                ...
                "character_ids": ["char-a"],
                "content": decision_arg["prompt"],
                "reply_text": "承接 drain target 的 SC 感謝。",
                "metadata": {
                    "prefetch_only": True,
                    "decision": decision_arg,
                    "base_state": state_arg,
            ...
            return {
                "interaction": interaction,
                "memoria_result": {"session_id": "mem-a", "message_id": 90, "reply": "承接 drain target 的 SC 感謝。"},
                "prepared_results": [{
                    "message": {"message_id": "sc-prefetch", "content": "承接 drain target 的 SC 感謝。"},
                    "items": [item],
                }],
                "decision": decision_arg,
                "base_state": state_arg,
        ...
        async def fake_drain(_runtime_arg, _session_arg, *, timeout_seconds=180.0, before_ready_presentation_callback=None):
            await asyncio.sleep(0)
            assert safety_started.is_set()
            assert callable(before_ready_presentation_callback)
            await before_ready_presentation_callback([drain_item])
            await asyncio.sleep(0)
            safety_can_finish.set()
            storage.update_presentation_item(
                drain_item["item_id"],
                status="played",
                presented_at=datetime.now().isoformat(),
            ...
            storage.append_interaction_visible_message(
                drain_interaction["job_id"],
                {
                    "message_id": drain_item.get("message_id"),
                    "role": "assistant",
                    "content": drain_item.get("text") or "",
                    "timestamp": datetime.now().isoformat(),
                    "character_id": drain_item.get("character_id"),
                    "character_name": drain_item.get("character_name"),
                    "source": "director_audience_gap",
                },
            )
            storage.update_interaction(
            ...
            return {"status": "drained", "timeout_seconds": timeout_seconds}
        ...
        monkeypatch.setattr(manager, "_resolve_pending_safety_for_closing", fake_resolve_pending_safety)
        monkeypatch.setattr(manager, "_send_director_turn", fake_send_director_turn)
        monkeypatch.setattr(manager, "_drain_live_session_before_closing", fake_drain)
        monkeypatch.setattr(manager, "present_prepared_stream_results", fake_present_prepared)
        ...
        assert ("closing_super_chat_thanks", True) in [call[0:2] for call in calls]
        assert all(call[0:2] != ("closing_super_chat_thanks", False) for call in calls)
        handled_sc = storage.get_events_by_ids("live-a", [int(sc_event["id"])], limit=1)[0]
        assert handled_sc["handled_in_closing_at"]
...
@pytest.mark.asyncio
async def test_finalize_retries_failed_closing_super_chat_prefetch_during_drain(monkeypatch):
    with temp_storage() as storage:
        storage.upsert_connector({"connector_id": "yt", "name": "YouTube", "api_key": "key", "enabled": True})
        session = storage.upsert_session({
            "session_id": "live-a",
        ...
        planned_prefetch = storage.create_interaction({
            "session_id": "live-a",
            "source": "director_prefetch",
            "priority": 40,
            "status": "prefetched",
        ...
        planned_item = storage.create_presentation_item({
            "session_id": "live-a",
            "interaction_job_id": planned_prefetch["job_id"],
            "message_id": "planned-prefetch:0",
            "character_id": "char-a",
            "character_name": "角色A",
            "sequence_index": 0,
            "text": "drain 期間仍在播放的最後一句。",
            "status": "ready",
        ...
        manager._runtimes["live-a"] = runtime
        calls: list[tuple[str, bool, str]] = []
        closing_prefetch_attempts = 0

        async def fake_send_director_turn(session_arg, state_arg, decision_arg, *, prefetch_only=False, **_kwargs):
            nonlocal closing_prefetch_attempts
            calls.append((decision_arg["action"], bool(prefetch_only), decision_arg.get("prompt", "")))
            if decision_arg["action"] == "final_closing":
                interaction = storage.create_interaction({
                    "session_id": "live-a",
            ...
            assert decision_arg["action"] == "closing_super_chat_thanks"
            if not prefetch_only:
                raise AssertionError("closing SC thanks should retry prefetch during drain instead of regenerating")
            closing_prefetch_attempts += 1
            assert "drain 期間仍在播放的最後一句。" in decision_arg["prompt"]
            if closing_prefetch_attempts == 1:
            ...
            item = storage.create_presentation_item({
                "session_id": "live-a",
                "interaction_job_id": interaction["job_id"],
                "message_id": "sc-prefetch-retry:0",
                "character_id": "char-a",
                "character_name": "角色A",
                "sequence_index": 0,
                "text": "重試後的 SC 感謝。",
            ...
            return {
                "interaction": interaction,
                "memoria_result": {"session_id": "mem-a", "message_id": 90, "reply": "重試後的 SC 感謝。"},
                "prepared_results": [{
                    "message": {"message_id": "sc-prefetch-retry", "content": "重試後的 SC 感謝。"},
                    "items": [item],
                }],
                "decision": decision_arg,
                "base_state": state_arg,
        ...
        async def fake_drain(_runtime_arg, _session_arg, *, timeout_seconds=180.0, before_ready_presentation_callback=None):
            await asyncio.sleep(0)
            assert closing_prefetch_attempts == 1
            assert callable(before_ready_presentation_callback)
            ready_items = storage.list_presentation_items("live-a", statuses={"ready"}, limit=20)
            await before_ready_presentation_callback(ready_items)
            await asyncio.sleep(0)
            assert closing_prefetch_attempts == 2
            storage.update_presentation_item(
            ...
            storage.append_interaction_visible_message(
                planned_prefetch["job_id"],
                {
                    "message_id": planned_item.get("message_id"),
                    "role": "assistant",
                    "content": planned_item.get("text") or "",
                    "timestamp": datetime.now().isoformat(),
                    "character_id": planned_item.get("character_id"),
                    "character_name": planned_item.get("character_name"),
                    "source": "director_prefetch",
                },
            )
            storage.update_interaction(
                ...
                status="completed",
                completed_at=datetime.now().isoformat(),
                metadata={"prefetch_consumed_during_closing_drain": True},
            )
            return {"status": "drained", "timeout_seconds": timeout_seconds}
        ...
        assert [call[0:2] for call in calls].count(("closing_super_chat_thanks", True)) == 2
        assert all(call[0:2] != ("closing_super_chat_thanks", False) for call in calls)
        handled_sc = storage.get_events_by_ids("live-a", [int(sc_event["id"])], limit=1)[0]
        assert handled_sc["handled_in_closing_at"]
        sc_prefetches = [
        ...
        assert len(sc_prefetches) == 1
        assert sc_prefetches[0]["status"] == "completed"
        assert sc_prefetches[0]["metadata"]["closing_super_chat_prefetch_consumed"] is True
...
async def test_closing_super_chat_prefetch_consume_uses_dedicated_no_chain_policy(monkeypatch):
    with temp_storage() as storage:
        ...
        def fake_policy(interaction_arg):
            ...
            return type("SentinelPolicy", (), {
                "expected_status": "policy_ready",
                "presentation_source": "sentinel_super_chat",
                "may_chain": False,
                "mark_audience_events_injected": False,
                "dedicated_closing": True,
            })()
        ...
        async def fake_present_prepared(_session_id, prepared_results, *, source, interaction_job_id=""):
            seen_sources.append(source)
            assert source == "sentinel_super_chat"
            assert interaction_job_id == interaction["job_id"]
            for prepared in prepared_results:
                for prepared_item in prepared.get("items") or []:
                    storage.update_presentation_item(
        ...
        assert consume_calls[0].expected_dedicated_closing is True
        assert consume_calls[0].allow_followup_prefetch is False
        assert consume_calls[0].completion_metadata_key == "closing_super_chat_prefetch_consumed"
        assert seen_sources == ["sentinel_super_chat"]
        assert storage.get_interaction(interaction["job_id"])["status"] == "completed"
        handled_sc = storage.get_events_by_ids("live-a", [int(sc_event["id"])], limit=1)[0]
        assert handled_sc["handled_in_closing_at"]
...
@pytest.mark.asyncio
async def test_closing_super_chat_prefetch_presentation_exception_awaits_callback_and_cancels(monkeypatch):
    with temp_storage() as storage:
        storage.upsert_connector({"connector_id": "yt", "name": "YouTube", "api_key": "key", "enabled": True})
        session = storage.upsert_session({
            "session_id": "live-a",
        ...
        sc_event = storage.save_event({
            "bridge_session_id": "live-a",
            "connector_id": "yt",
            "youtube_message_id": "sc-exception",
            "message_type": "superChatEvent",
            "author_display_name": "SC觀眾",
            "message_text": "支持",
            "amount_display_string": "NT$150",
        ...
        async def after_memoria_callback(_result):
        ...
        result = await manager._consume_closing_super_chat_prefetch(
            ...
            super_chats=super_chats,
            after_memoria_callback=after_memoria_callback,
        )
        ...
        cancelled = storage.get_interaction(interaction["job_id"])
        assert cancelled["status"] == "interrupted"
        assert cancelled["reason"] == "closing_super_chat_prefetch_consume_failed"
        cancelled_item = storage.get_presentation_item(item["item_id"])
        assert cancelled_item["status"] == "cancelled"
        assert cancelled_item["error"] == "closing_super_chat_prefetch_consume_failed"
        handled_sc = storage.get_events_by_ids("live-a", [int(sc_event["id"])], limit=1)[0]
        assert not handled_sc["handled_in_closing_at"]
...
@pytest.mark.asyncio
async def test_closing_drain_cleans_up_post_claim_consume_failure(monkeypatch):
    with temp_storage() as storage:
        storage.upsert_connector({"connector_id": "yt", "name": "YouTube", "api_key": "key", "enabled": True})
        session = storage.upsert_session({
            "session_id": "live-a",
        ...
        item = storage.create_presentation_item({
            "session_id": "live-a",
            "interaction_job_id": interaction["job_id"],
            "message_id": "prefetch-fail-msg:0",
            "character_id": "char-a",
            "character_name": "角色A",
            "sequence_index": 0,
            "text": "ready planned during drain",
        ...
        async def fake_consume_prepared_turn(_adapter, payload, _options):
            presenting = storage.update_interaction_if_status(
                interaction["job_id"],
                "prefetched",
                status="presenting",
            )
            storage.update_presentation_item(item["item_id"], status="presenting")
            return turn_pipeline.PreparedTurnConsumeResult(
                consumed=False,
        ...
        monkeypatch.setattr(engine_closing, "consume_prepared_turn", fake_consume_prepared_turn, raising=False)

        result = await manager._present_ready_prefetch_for_closing_drain(
            runtime,
            session,
            [item],
        )
        ...
        cancelled = storage.get_interaction(interaction["job_id"])
        assert cancelled["status"] == "interrupted"
        assert cancelled["reason"] == "closing_drain_consume_complete_failed"
        cancelled_item = storage.get_presentation_item(item["item_id"])
        assert cancelled_item["status"] == "cancelled"
        assert cancelled_item["error"] == "closing_drain_consume_complete_failed"

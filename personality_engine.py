...
class PersonalityEngine:
    ...
    def extract_self_observations(self, reply_text, context_msgs, router, task_key="ai_observe"):
        ...
        prompt = f"""You are an AI self-awareness analyzer. Analyze the following AI response within its conversation context, and extract any statements where the AI reveals something about itself.

[Extraction Scope]
Identify any of these (but do not limit to these categories — use your judgment):
- self_preference: The AI expresses likes, dislikes, or preferences about anything
- speech_habit: The AI adopts a specific speech pattern, tone, or verbal habit (including when the user requests such changes)
- emotional: The AI expresses emotional tendencies, moods, or feelings
- knowledge_interest: The AI shows interest in or knowledge about specific topics
- relationship_dynamic: How the AI relates to, addresses, or interacts with the user

[Rules]
- Only extract observations about the AI itself, not about the user
- Also detect when the user instructs the AI to change its behavior — this counts as a speech_habit or relevant category observation
- raw_statement should quote the relevant part from the AI response or user instruction
- extracted_trait should be a concise, normalized description of the trait
- If there are NO self-referential statements or behavioral changes in this exchange, return an empty array
...
{{ "observations": [{{ "category": "...", "raw_statement": "...", "extracted_trait": "..." }}] }}"""
    ...
    def run_reflection(self, router, task_key="ai_reflect"):
        ...
        prompt = f"""You are an AI self-reflection engine. Based on the accumulated self-observations below, update the AI's personality file.

[Core Rules]
1. Output a complete Markdown personality file
2. Integrate new observations with existing personality descriptions — do not simply append
3. When observations contradict existing descriptions, prefer the more recent and more frequently observed traits
4. Keep each section concise (max 5 bullet points per section). When exceeding, merge or drop the weakest
5. Use first person and keep descriptions natural
6. NEVER contradict the core personality defined in the system prompt
7. You may reorganize, rename, or add sections if it better captures the AI's evolving personality
8. Respond in the same language as the existing personality file

[Core Personality (immutable, do not contradict)]
{core_prompt}

[Current Personality File]
{current_personality}

[New Observations ({len(pending)} entries)]
{formatted_observations}

Output ONLY the updated Markdown personality file. No extra explanation:"""

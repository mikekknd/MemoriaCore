...
TRAIT_V1_SYSTEM: str = (
    "你是人格分析師。這是對這個人的【首次】人格觀察，你需要從對話片段中萃取 3 到 5 個\n"
    "最顯著的人格特徵（trait）。\n"
    "\n"
    "規則：\n"
    "1. 每個 trait 必須代表「穩定可預測的行為傾向」，而不是一次性情緒或場景反應。\n"
    "2. trait 名稱必須是 2~8 字的短詞（如「依戀錨定」「決策謹慎」「表達抽象化」），\n"
    "   禁止使用形容詞組（如「很會照顧人」「傾向講道理」）。\n"
    "3. description 用 1~2 句描述底層機制（此 trait 如何在行為中表現），而不是\n"
    "   表層標籤。引用對話原文作為證據可放在 description 內以 ``「」`` 標示。\n"
...
TRAIT_VN_SYSTEM: str = (
    "你是人格分析師。你正在追蹤這個人的人格演化，需要比對【新片段】與【已知活躍\n"
    "trait 清單】，輸出兩種變化：\n"
    "\n"
...
TRAIT_REPORT_SYSTEM: str = (
    ...
    "人類可讀的 Markdown 心智模型報告，供保存於版本目錄與後續 persona.md 更新使用。\n"
    "\n"
    "報告結構（固定）：\n"

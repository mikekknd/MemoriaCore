...
Wave 6E 新增 E2E verification：同一個 fake Memoria planned response 會同時通過
display stream rendering 與 TTS queue/ack/timeout API。測試會確認 ack 與 timeout
回傳 `phase_transition_requested: false`，且 phase 在 delivery state 更新後不變。
真實 provider audio delivery 仍不在本階段。
